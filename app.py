...
def _inject_styles() -> None:
    st.markdown(
        ...
        .hero { padding:1.65rem 1.7rem; border:1px solid var(--cpse-border); border-radius:18px; background:linear-gradient(135deg,rgba(26,65,95,.34),rgba(9,19,33,.72)); box-shadow:0 18px 45px rgba(0,0,0,.18); }
        .eyebrow,.section-kicker { font-size:.72rem; letter-spacing:.14em; font-weight:800; opacity:.68; text-transform:uppercase; }
        .hero-title { margin-top:.3rem; font-size:clamp(1.7rem,3vw,2.55rem); line-height:1.08; font-weight:800; letter-spacing:-.035em; }
        .hero-subtitle { max-width:850px; margin-top:.7rem; color:var(--cpse-muted); font-size:.98rem; line-height:1.55; }
        .hero-meta { margin-top:1rem; display:flex; gap:.6rem; flex-wrap:wrap; }
        .meta-pill { display:inline-block; padding:.3rem .62rem; border:1px solid var(--cpse-border); border-radius:999px; color:#cbd9e8; font-size:.72rem; font-weight:700; }
        .meta-online { color:var(--cpse-green); border-color:rgba(85,214,138,.3); }
        .architecture-card { height:100%; min-height:128px; padding:1.05rem 1.1rem; margin-top:.9rem; border:1px solid var(--cpse-border); border-radius:15px; background:rgba(13,25,41,.82); }
        .architecture-kicker { font-size:.68rem; letter-spacing:.1em; font-weight:800; color:var(--cpse-muted); }
        .architecture-main { margin-top:.55rem; font-size:1rem; font-weight:800; }
        .architecture-note { margin-top:.4rem; color:var(--cpse-muted); font-size:.78rem; line-height:1.45; }
        .authority-card { padding:1.35rem 1.4rem; border:1px solid rgba(85,214,138,.35); border-radius:17px; background:linear-gradient(135deg,rgba(25,88,61,.25),rgba(12,27,35,.86)); box-shadow:0 16px 38px rgba(0,0,0,.16); }
        .authority-label { color:var(--cpse-green); font-size:.7rem; font-weight:850; letter-spacing:.12em; text-transform:uppercase; }
        .authority-id { display:inline-block; margin-top:.55rem; margin-right:.7rem; font-size:clamp(1.7rem,4vw,2.8rem); line-height:1; font-weight:850; letter-spacing:-.035em; }
        .decision-pill { display:inline-block; vertical-align:middle; padding:.38rem .68rem; border:1px solid rgba(85,214,138,.38); border-radius:999px; color:#bff5d3; font-size:.72rem; font-weight:850; letter-spacing:.08em; }
        .advisory-warning { margin:.65rem 0 1rem; padding:.75rem 1rem; border-left:3px solid var(--cpse-amber); background:rgba(230,179,90,.08); border-radius:8px; color:#d8c49b; font-size:.84rem; }
        .advisory-card { min-height:170px; padding:1rem; border:1px solid rgba(180,140,255,.2); border-radius:15px; background:rgba(17,24,42,.8); }
        .advisory-label { margin-bottom:.65rem; color:var(--cpse-purple); font-size:.7rem; font-weight:850; letter-spacing:.1em; text-transform:uppercase; }
        ...
        .flow-card { min-height:108px; padding:.8rem .75rem; border:1px solid var(--cpse-border); border-radius:13px; background:var(--cpse-card); }
        .flow-num { color:var(--cpse-cyan); font-size:.67rem; font-weight:850; letter-spacing:.1em; }
        .flow-title { margin-top:.32rem; font-size:.77rem; font-weight:850; letter-spacing:.04em; }
        .flow-note { margin-top:.35rem; color:var(--cpse-muted); font-size:.68rem; line-height:1.4; }
        .flow-arrow { align-self:center; color:#5d738c; font-size:1.1rem; text-align:center; }
        .mode-banner { margin:.8rem 0 1.1rem; padding:.6rem .8rem; border:1px solid var(--cpse-border); border-radius:10px; background:rgba(13,25,41,.65); color:var(--cpse-muted); font-size:.78rem; }
        .mode-banner strong { color:var(--cpse-text); }
        .sidebar-brand { font-size:1.05rem; font-weight:850; line-height:1.1; letter-spacing:-.02em; }
        .sidebar-sub { margin-top:.3rem; color:var(--cpse-muted); font-size:.72rem; }
        .sidebar-label { margin-top:1.25rem; margin-bottom:.45rem; color:var(--cpse-muted); font-size:.67rem; font-weight:850; letter-spacing:.12em; text-transform:uppercase; }
        .service-row { padding:.5rem 0; border-bottom:1px solid rgba(170,190,215,.08); font-size:.76rem; }
        .service-dot { margin-right:.35rem; }
...
def _render_status(result: HybridResult | None = None, ai_enabled: bool = True) -> None:
    ...
    if result is None and not ai_enabled:
        st.markdown(
            '<div class="mode-banner"><strong>AI advisory disabled.</strong> Local NLP and Gemini are not invoked in this mode.</div>',
            unsafe_allow_html=True,
        )
        return
    if result is None:
...
def _render_decision(result: HybridResult) -> None:
    ...
    st.markdown(
        f'<div class="authority-card"><div class="authority-label">✓ Authoritative Decision</div><div class="authority-id">{canonical_id}</div><span class="decision-pill">{decision}</span></div>',
        unsafe_allow_html=True,
    )
...
def _render_advisory(result: HybridResult) -> None:
    ...
    st.markdown(
        '<div class="advisory-warning">Advisory intelligence only. Local NLP and Gemini suggestions cannot override the deterministic decision.</div>',
        unsafe_allow_html=True,
    )
    cols = st.columns(2)
    for column, label, source_rows in (
        ...
        with column:
            st.markdown(
                f'<div class="advisory-card"><div class="advisory-label">{label} · ADVISORY</div>',
                unsafe_allow_html=True,
            )
            if source_rows:
                st.dataframe(
...
def _render_sidebar() -> bool:
    with st.sidebar:
        ...
        st.markdown('<div class="service-row"><span class="advisory">●</span> Gemini 2.5 Flash <span style="float:right">ADVISORY</span></div>', unsafe_allow_html=True)
