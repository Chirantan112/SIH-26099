...
def _safe_suggestions(component: str, adapter: _SuggestionAdapter, operation: Callable[[_SuggestionAdapter], tuple[CandidateSuggestion, ...]], status: AdapterStatus) -> tuple[tuple[CandidateSuggestion, ...], AdapterStatus]:
...
def _valid_suggestions(suggestions: tuple[CandidateSuggestion, ...], catalog: tuple[CatalogRecord, ...]) -> tuple[CandidateSuggestion, ...]:
    ...
    for suggestion in suggestions:
        if not isinstance(suggestion, CandidateSuggestion):
            continue
        if (suggestion.canonical_material_id not in known_ids or not isinstance(suggestion.score, float) or not isfinite(suggestion.score) or not isinstance(suggestion.source, str) or not suggestion.source.strip() or not isinstance(suggestion.explanation, str) or not isinstance(suggestion.matching_attributes, tuple) or not isinstance(suggestion.conflicting_attributes, tuple) or not isinstance(suggestion.missing_attributes, tuple) or suggestion.technical_compatible not in (True, False, None)):
            continue
...
def _evidence_compatible(item: CandidateSuggestion) -> bool:
    """Only explicit technical compatibility can produce MATCHED consensus."""
    return item.technical_compatible is True


def _has_explicit_conflict(suggestions: tuple[CandidateSuggestion, ...]) -> bool:
    return bool(suggestions) and all(item.technical_compatible is False and bool(item.conflicting_attributes) for item in suggestions)


def _single_advisor_consensus(source_label: str, candidates: tuple[CandidateSuggestion, ...], local_ids: tuple[str, ...], gemini_ids: tuple[str, ...]) -> AIConsensus:
    compatible = tuple(item for item in candidates if _evidence_compatible(item))
    if compatible:
        return AIConsensus("MATCHED", compatible[0].canonical_material_id, f"Only {source_label} supplied usable advisory evidence; its technical evidence supports a compatible candidate.", local_ids, gemini_ids)
    if _has_explicit_conflict(candidates):
        return AIConsensus("NEW_CANDIDATE", None, f"Only {source_label} supplied usable advisory evidence; it explicitly rules out every supplied candidate with technical conflicts.", local_ids, gemini_ids)
    return AIConsensus("UNCERTAIN", None, f"Only {source_label} supplied usable advisory evidence; its technical evidence is incomplete or unresolved.", local_ids, gemini_ids)


def _ai_consensus(statuses: tuple[AdapterStatus, AdapterStatus], suggestions: tuple[CandidateSuggestion, ...]) -> AIConsensus:
    ...
    if local_evidence_available and gemini_evidence_available:
        if local_compatible and gemini_compatible:
            if local_compatible[0].canonical_material_id == gemini_compatible[0].canonical_material_id:
                return AIConsensus("MATCHED", local_compatible[0].canonical_material_id, "Local NLP and Gemini independently identify the same technically compatible candidate with no reported critical conflict.", local_ids, gemini_ids)
...
def run_hybrid_pipeline(raw_description: str | None, catalog: tuple[CatalogRecord, ...], legacy_material_code: str = "HYBRID-INPUT", retrieval_adapter: RetrievalAdapter | None = None, llm_adapter: LLMInterpretationAdapter | None = None, progress_callback: Callable[[str], None] | None = None) -> HybridResult:
    ...
    retrieval_suggestions, retrieval_status = _safe_suggestions("local_nlp", retrieval, lambda adapter: adapter.retrieve(raw_description, normalization.normalized_text, extraction.attributes, catalog), retrieval_status)
    llm_suggestions, llm_status = _safe_suggestions("llm", llm, lambda adapter: adapter.interpret(raw_description, normalization.normalized_text, extraction.attributes, catalog), llm_status)
    _report("ai_advisory")
    ...
    fallback_information = "Deterministic LEGO #2-#5 mapping remains active; unavailable or failed AI adapters were skipped." if fallback_used else "Deterministic LEGO #2-#5 mapping remains authoritative; AI consensus is advisory evidence only."
    result = HybridResult(legacy_material_code, normalization.normalized_text, normalization.transformations, extraction.attributes, mapping_result, statuses, suggestions, consensus, fallback_used, fallback_information, "AI suggestions and consensus are advisory only; final mapping is the unchanged deterministic LEGO #4/#5 result.")
    _report("decide")
    return result
