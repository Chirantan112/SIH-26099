...
class AIConsensusTests(unittest.TestCase):
    def test_equivalent_candidate_is_matched_when_both_ai_components_agree(self):
        ...
        local = evidence("local_embedding", "VAL-001", True, fields)
        gemini = evidence("gemini", "VAL-001", True, fields)
    ...
    def test_ai_false_negative_cannot_create_new_candidate(self):
    ...
    def test_one_ai_unavailable_and_other_incomplete_is_uncertain(self):
        local = evidence("local_embedding", "VAL-001", None, missing=("pressure_class",))
    ...
    def test_one_ai_unavailable_and_other_compatible_remains_uncertain(self):
        local = evidence("local_embedding", "VAL-001", True)
        result = run_hybrid_pipeline("CS GATE VLV 50MM FLG CL150", CATALOG, retrieval_adapter=RetrievalStub((local,)), llm_adapter=GeminiStub(available=False))
        self.assertEqual(result.ai_consensus.conclusion, "UNCERTAIN")
        self.assertEqual(result.ai_consensus.canonical_material_id, "VAL-001")
    ...
    def test_local_technical_evidence_exact_agreement_is_true(self):
        ...
        right = extract_attributes("CS GATE VLV 50MM FLG CL150").attributes
        matching, conflicts, missing, compatible = LocalEmbeddingRetrievalAdapter._technical_evidence(left, right)
        self.assertTrue(compatible)
        self.assertIn("pressure_class", matching)
        self.assertEqual(conflicts, ())
        self.assertEqual(missing, ())

    def test_local_technical_evidence_critical_conflict_is_false(self):
        left = extract_attributes("CS GATE VLV 50MM FLG CL150").attributes
        right = extract_attributes("CS GATE VLV 50MM FLG CL300").attributes
        _matching, conflicts, _missing, compatible = LocalEmbeddingRetrievalAdapter._technical_evidence(left, right)
        self.assertFalse(compatible)
        self.assertIn("pressure_class", conflicts)

    def test_local_technical_evidence_missing_critical_attribute_is_none(self):
        left = extract_attributes("CS GATE VLV 50MM FLG").attributes
        right = extract_attributes("CS GATE VLV 50MM FLG CL150").attributes
        _matching, _conflicts, missing, compatible = LocalEmbeddingRetrievalAdapter._technical_evidence(left, right)
        self.assertIsNone(compatible)
        self.assertIn("pressure_class", missing)

    def test_local_technical_evidence_different_category_is_false(self):
        left = MaterialAttributes(category="Valve")
        right = MaterialAttributes(category="Pipe")
        _matching, conflicts, _missing, compatible = LocalEmbeddingRetrievalAdapter._technical_evidence(left, right)
        self.assertFalse(compatible)
        self.assertIn("category", conflicts)

    def test_local_technical_evidence_without_usable_category_is_none(self):
        left = MaterialAttributes()
        right = MaterialAttributes()
        _matching, conflicts, missing, compatible = LocalEmbeddingRetrievalAdapter._technical_evidence(left, right)
        self.assertIsNone(compatible)
        self.assertEqual(conflicts, ())
        self.assertEqual(missing, ())
